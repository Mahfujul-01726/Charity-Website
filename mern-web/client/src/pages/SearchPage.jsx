...
export default function SearchPage() {
    ...
    const results = useMemo(() => {
        ...
        return {
            categories: donationCategories.filter(
    ...
    return (
        <section className="container section">
            <article className="glass-card" style={{ marginBottom: '40px', padding: '40px' }}>
                <h2 style={{ textAlign: 'center', marginBottom: '24px' }}>Discover Campaigns & Causes</h2>
                <div style={{ position: 'relative', maxWidth: '700px', margin: '0 auto 24px' }}>
                    <i className="ph ph-magnifying-glass" style={{ position: 'absolute', left: '20px', top: '50%', transform: 'translateY(-50%)', fontSize: '1.5rem', color: 'var(--text-secondary)' }}></i>
                    <input
                        placeholder="Search for 'Medical', 'Education', 'Disaster Relief'..."
                        value={query}
                        onChange={(e) => setQuery(e.target.value)}
                        style={{ paddingLeft: '56px', paddingRight: '24px', fontSize: '1.1rem', height: '60px', borderRadius: '30px' }}
                    />
                    {query && (
                        <button 
                            onClick={() => setQuery("")}
                            style={{ position: 'absolute', right: '20px', top: '50%', transform: 'translateY(-50%)', background: 'transparent', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '1.2rem' }}
                        >
            ...
            {query && (
                <>
                    <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px' }}>
                        <i className="ph ph-squares-four"></i> Category Results ({results.categories.length})
                    </h3>
                    <div className="category-grid" style={{ marginBottom: '40px' }}>
                        {results.categories.map((item) => (
                            <Link key={item.slug} to={item.slug === "ngo" ? "/ngo-locator" : `/donate/${item.slug}`} className="category-card" style={{ "--cat-color": item.color }}>
                        ...
                        {results.categories.length === 0 && (
                            <p className="meta-text">No matching categories found.</p>
                        )}
                    </div>

                    <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px' }}>
                        <i className="ph ph-megaphone"></i> Campaign Results ({results.campaigns.length})
                    </h3>
                    <div className="card-grid">
                        ...
                        {results.campaigns.length === 0 && (
                            <p className="meta-text" style={{ gridColumn: '1 / -1', padding: '40px', textAlign: 'center', background: 'var(--glass-bg)', borderRadius: '24px', border: '1px solid var(--glass-border)' }}>
                                ...
                                No matching campaigns found. Try adjusting your search query.
                            </p>
                        )}
                    </div>
                </>
